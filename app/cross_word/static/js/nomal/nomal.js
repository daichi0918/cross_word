
window.onload = function() {
    // alert("dddddddddddd");
// 空白は全角で空ける
// 値を渡すことができない場合処理が途中で止まる
    document.getElementById("level").textContent="中級　その壱";

    document.getElementById("one").textContent="　";
    document.getElementById("one-s").textContent="　";

    document.getElementById("two").textContent="A";
    document.getElementById("two-s").textContent="　";

    document.getElementById("three").textContent="　";
    document.getElementById("three-s").textContent="　";

    document.getElementById("four").textContent="B";
    document.getElementById("four-s").textContent="　";

    document.getElementById("five").textContent="　";
    document.getElementById("five-s").textContent="　";

    document.getElementById("six").textContent="　";
    document.getElementById("six-s").textContent="　";

    document.getElementById("seven").textContent="　";
    document.getElementById("seven-s").textContent="　";

    document.getElementById("eight").textContent="　";
    document.getElementById("eight-s").textContent="　";

    document.getElementById("nine").textContent="C";
    document.getElementById("nine-s").textContent="　";

    document.getElementById("ten").textContent="　";
    document.getElementById("ten-s").textContent="　";
    
    document.getElementById("eleven").textContent="　";
    document.getElementById("eleven-s").textContent="　";
    
    document.getElementById("twelve").textContent="　";
    document.getElementById("twelve-s").textContent="2";
    
    document.getElementById("thirteen").textContent="　";
    document.getElementById("thirteen-s").textContent="　";
    
    document.getElementById("fourteen").textContent="　";
    document.getElementById("fourteen-s").textContent="1";
    
    document.getElementById("fifteen").textContent="　";
    document.getElementById("fifteen-s").textContent="　";
    
    document.getElementById("sixteen").textContent="　";
    document.getElementById("sixteen-s").textContent="3";
    

    document.getElementById("one-value").textContent="　";
    document.getElementById("two-value").textContent="フ";
    document.getElementById("three-value").textContent="　";
    document.getElementById("four-value").textContent="リ";
    document.getElementById("five-value").textContent="　";
    document.getElementById("six-value").textContent="レ";
    document.getElementById("seven-value").textContent="　";
    document.getElementById("eight-value").textContent="モ";
    document.getElementById("nine-value").textContent="ル";
    document.getElementById("ten-value").textContent="ー";
    document.getElementById("eleven-value").textContent="タ";
    document.getElementById("twelve-value").textContent="ー";
    document.getElementById("thirteen-value").textContent="　";
    document.getElementById("fourteen-value").textContent="ム";
    document.getElementById("fifteen-value").textContent="　";
    document.getElementById("sixteen-value").textContent="ト";

};

// static/js/hard/hard.js