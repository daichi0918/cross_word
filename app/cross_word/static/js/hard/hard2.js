
window.onload = function() {
    // alert("dddddddddddd");
// 空白は全角で空ける
// 値を渡すことができない場合処理が途中で止まる
    document.getElementById("level").textContent="上級　その弐";

    document.getElementById("one").textContent="A";
    document.getElementById("one-s").textContent="(7)";

    document.getElementById("two").textContent="B";
    document.getElementById("two-s").textContent="　";

    document.getElementById("three").textContent="　";
    document.getElementById("three-s").textContent="　";

    document.getElementById("four").textContent="　";
    document.getElementById("four-s").textContent="　";

    document.getElementById("five").textContent="C";
    document.getElementById("five-s").textContent="(3)";

    document.getElementById("six").textContent="　";
    document.getElementById("six-s").textContent="　";

    document.getElementById("seven").textContent="　";
    document.getElementById("seven-s").textContent="(1)";

    document.getElementById("eight").textContent="　";
    document.getElementById("eight-s").textContent="　";

    document.getElementById("nine").textContent="D";
    document.getElementById("nine-s").textContent="　";

    document.getElementById("ten").textContent="　";
    document.getElementById("ten-s").textContent="　";
    
    document.getElementById("eleven").textContent="E";
    document.getElementById("eleven-s").textContent="　";
    
    document.getElementById("twelve").textContent="　";
    document.getElementById("twelve-s").textContent="　";
    
    document.getElementById("thirteen").textContent="F";
    document.getElementById("thirteen-s").textContent="　";
    
    document.getElementById("fourteen").textContent="　";
    document.getElementById("fourteen-s").textContent="　";
    
    document.getElementById("fifteen").textContent="　";
    document.getElementById("fifteen-s").textContent="　";
    
    document.getElementById("sixteen").textContent="　";
    document.getElementById("sixteen-s").textContent="(4)";
    
    document.getElementById("seventeen").textContent="　";
    document.getElementById("seventeen-s").textContent="　";
    
    document.getElementById("eighteen").textContent="G";
    document.getElementById("eighteen-s").textContent="　";
    
    document.getElementById("nineteen").textContent="H";
    document.getElementById("nineteen-s").textContent="　";
    
    document.getElementById("twenty").textContent="　";
    document.getElementById("twenty-s").textContent="　";
    
    document.getElementById("twenty-one").textContent="I";
    document.getElementById("twenty-one-s").textContent="　";
    
    document.getElementById("twenty-two").textContent="　";
    document.getElementById("twenty-two-s").textContent="(6)";
    
    document.getElementById("twenty-three").textContent="　";
    document.getElementById("twenty-three-s").textContent="　";
    
    document.getElementById("twenty-four").textContent="J";
    document.getElementById("twenty-four-s").textContent="(2:5)";
    
    document.getElementById("twenty-five").textContent="　";
    document.getElementById("twenty-five-s").textContent="　";



    document.getElementById("one-value").textContent="T";
    document.getElementById("two-value").textContent="C";
    document.getElementById("three-value").textContent="P";
    document.getElementById("four-value").textContent="　";
    document.getElementById("five-value").textContent="R";
    document.getElementById("six-value").textContent="　";
    document.getElementById("seven-value").textContent="P";
    document.getElementById("eight-value").textContent="　";
    document.getElementById("nine-value").textContent="M";
    document.getElementById("ten-value").textContent="O";
    document.getElementById("eleven-value").textContent="R";
    document.getElementById("twelve-value").textContent="I";
    document.getElementById("thirteen-value").textContent="P";
    document.getElementById("fourteen-value").textContent="　";
    document.getElementById("fifteen-value").textContent="M";
    document.getElementById("sixteen-value").textContent="F";
    document.getElementById("seventeen-value").textContent="　";
    document.getElementById("eighteen-value").textContent="M";
    document.getElementById("nineteen-value").textContent="S";
    document.getElementById("twenty-value").textContent="　";
    document.getElementById("twenty-one-value").textContent="P";
    document.getElementById("twenty-two-value").textContent="C";
    document.getElementById("twenty-three-value").textContent="　";
    document.getElementById("twenty-four-value").textContent="E";
    document.getElementById("twenty-five-value").textContent="R";

};

